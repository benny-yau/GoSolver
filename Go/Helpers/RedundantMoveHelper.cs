...
namespace Go
{
    public class RedundantMoveHelper
    {
        ...
        /// Ensure any previous group is non killable <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_GuanZiPu_A17_2" />
        ...
        /// Move group more than two points <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_WuQingYuan_Q31680_2" />
        /// </summary>
        public static Boolean SuicidalConnectAndDie(GameTryMove tryMove)
        {
            ...
            //move group more than two points
            if (tryBoard.MoveGroup.Points.Count == 1) return false;
            if (tryBoard.MoveGroup.Points.Count == 2 && (!tryBoard.MoveGroup.Points.Any(p => tryBoard.CornerPoint(p)) || !tryBoard.GetGroupLibertyPoints(tryBoard.MoveGroup).Any(q => ImmovableHelper.FindTigerMouth(tryBoard, c, q)))) return false;
            if (AtariHelper.AtariByGroup(tryBoard, tryBoard.MoveGroup)) return false;
            ...
            {
                //ensure any previous group is non killable
                if (LinkHelper.GetPreviousMoveGroup(currentBoard, tryBoard).Any(group => WallHelper.IsNonKillableGroup(currentBoard, group)))
                {
                    //check killer formation
                    return !KillerFormationHelper.SuicidalKillerFormations(tryBoard, currentBoard);
                }
            }
            else
            {
                IEnumerable<Group> groups = tryBoard.GetNeighbourGroups(tryBoard.MoveGroup);
                //ensure at least one non killable group
                if (groups.All(group => !WallHelper.IsNonKillableGroup(tryBoard, group))) return false;
                //ensure no killable group with two or less liberties
                if (groups.Where(group => tryBoard.GetGroupLiberties(group) <= 2).Any(group => !WallHelper.IsNonKillableGroup(tryBoard, group)))
                    return false;
                return true;
            }
            return false;
        ...
        private static Boolean RedundantSuicideNearNonKillableGroup(GameTryMove tryMove, Board capturedBoard)
        {
            ...
            if (GameHelper.GetContentForSurviveOrKill(tryBoard.GameInfo, SurviveOrKill.Survive) == c)
            {
                ...
                {
                    //check connect and die
                    if (ImmovableHelper.AllConnectAndDie(capturedBoard, p))
                        return false;
                    return true;
                }
                return false;
        ...
        public static Boolean SurvivalLeapMove(GameTryMove tryMove)
        {
            ...
            if (closestNeighbours.Count == 0) return true;
        ...
        /// Test move with liberty of one <see cref="UnitTestProject.MustHaveNeutralMoveTest.MustHaveNeutralMoveTest_Scenario_XuanXuanGo_A23" />
        /// Is suicide move for both current board and try board <see cref="UnitTestProject.MustHaveNeutralMoveTest.MustHaveNeutralMoveTest_Scenario_GuanZiPu_Weiqi101_19138" />
        /// </summary>
        ...
        public static Boolean IsImmovableKill(Game g, List<Group> killerGroups)
        {
            ...
            //killer liberties that connect to more than one of survival groups
            if (killerLiberties.Any(liberty => g.Board.GetGroupsFromStoneNeighbours(liberty, killerGroup.Content).Count > 1))
                return false;
            return true;
        }
        ...
        /// <summary>
        /// Find survival moves at tiger mouth that are redundant, and opponent suicide point for sorting game try moves.
        ...
        public static Boolean RedundantTigerMouthMove(GameTryMove tryMove)
        {
            ...
            if (RedundantTigerMouth(opponentMove))
                return true;
        ...
        /// </summary>
        private static Boolean RedundantTigerMouth(GameTryMove tryMove)
        {
            Point move = tryMove.Move;
            Board tryBoard = tryMove.TryGame.Board;
            Board currentBoard = tryMove.CurrentGame.Board;
            ...
            {
                Group killerGroup = BothAliveHelper.GetKillerGroupFromCache(currentBoard, eyePoint);
        ...
        public static Boolean GenericEyeFillerMove(GameTryMove tryMove)
        {
            ...
            foreach (Point p in emptyNeighbours)
            {
                ...
                {
                    (Boolean suicidal, Board b) = ImmovableHelper.IsSuicidalMove(p, c, currentBoard);
                    if (suicidal) continue;
                    //check any opponent stone at neighbour points
                    if (currentBoard.GetStoneNeighbours(p.x, p.y).Any(n => currentBoard[n] == c.Opposite()))
                        continue;
        ...
        public static Boolean PossibilityOfDoubleKo(GameTryMove tryMove)
        {
            ...
            if (!isSurvive)
            {
                ...
                HashSet<Point> liberties = currentBoard.GetLibertiesOfGroups(groups.ToList());
                foreach (Point liberty in liberties)
                {
                    if (EyeHelper.FindEye(currentBoard, liberty, groups.First().Content))
                        currentBoard.GetStoneNeighbours(liberty.x, liberty.y).Where(n => currentBoard.GetGroupAt(n).Points.Count == 1).ToList().ForEach(x => targetGroups.Add(currentBoard.GetGroupAt(x)));
                }
            }
