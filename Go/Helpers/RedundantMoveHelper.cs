...
namespace Go
{
    public class RedundantMoveHelper
    {
        ...
        /// Check eye at eye point <see cref="UnitTestProject.CoveredEyeMoveTest.CoveredEyeMoveTest_Scenario_XuanXuanGo_A26" /> 
        /// Check no eye for survival for opponent <see cref="UnitTestProject.CoveredEyeMoveTest.CoveredEyeMoveTest_Scenario_Corner_B2" /> 
        /// Check eye for ko fight <see cref="UnitTestProject.CoveredEyeMoveTest.CoveredEyeMoveTest_Scenario_WuQingYuan_Q30982" /> 
        /// Check eye for survival <see cref="UnitTestProject.CoveredEyeMoveTest.CoveredEyeMoveTest_Scenario_XuanXuanGo_A34" />
        ...
        public static Boolean FindCoveredEyeMove(GameTryMove tryMove, GameTryMove opponentTryMove = null)
        {
            ...
            if (DiagonalNonSemiSolidEye(currentBoard, tryBoard, eyeGroup))
                return false;
            ...
            //check eye for ko fight
            if (eyeGroup.Points.Count == 1 && tryBoard.GetStoneAndDiagonalNeighbours(eyePoint.x, eyePoint.y).Any(n => !WallHelper.NoEyeForSurvival(tryBoard, n, c)))
                return false;
        ...
        /// </summary>
        public static Boolean DiagonalNonSemiSolidEye(Board currentBoard, Board tryBoard, Group eyeGroup)
        {
        ...
        private static Boolean MultiPointOpponentSuicidalMove(GameTryMove tryMove, GameTryMove opponentMove)
        {
            ...
            foreach (Group atariTarget in tryBoard.AtariTargets)
            {
                foreach (Group neighbourGroup in tryBoard.GetNeighbourGroups(atariTarget).Where(group => group.Liberties.Count == 1))
                {
                    ...
                    if (b != null && (b.MoveGroupLiberties == 1 || ImmovableHelper.CheckConnectAndDie(b) || ImmovableHelper.CheckSnapbackInNeighbourGroups(b, b.CapturedList.First())))
                        return false;
                }
            }
        ...
        private static Boolean FillerMoveWithoutKillerGroup(GameTryMove tryMove, GameTryMove opponentMove = null)
        {
            ...
            return GenericEyeFillerMove(tryMove, opponentMove);
        }
