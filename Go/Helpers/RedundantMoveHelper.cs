    ...
    public class RedundantMoveHelper
    {
        ...
        public static Boolean FindPotentialEye(GameTryMove tryMove)
        {
            ...
            if (!EyeHelper.IsCovered(currentBoard, move, c))
            {
                ...
                if (tryBoard.MoveGroupLiberties == 1 && tryMove.EssentialSuicidalKillerFormation)
                    return false;
        ...
        public static Boolean FindCoveredEyeMove(GameTryMove tryMove, GameTryMove opponentMove = null)
        {
            ...
            if (tryMove.PossibleLinkForGroups)
                return false;
            ...
            //check ko fight
            if (currentBoard.CornerPoint(move) && KoHelper.MakeKoFightFromEyePoint(currentBoard, move, c.Opposite(), false).Item1)
                return false;
        ...
        public static Boolean FillKoEyeMove(GameTryMove tryMove)
        {
            ...
            if (tryMove.MoveConnectAndDie)
            {
                ...
                if (tryMove.EssentialSuicidalKillerFormation)
                    return false;
            ...
            if (isKoFight)
            {
                if (!KoHelper.KoContentEnabled(c, tryBoard.GameInfo))
                    return false;
                //check for target and survival points
                if (tryMove.OpponentMove != null && LifeCheck.CheckIfDeadOrAlive(SurviveOrKill.Kill, tryMove.OpponentMove.TryGame.Board) == ConfirmAliveResult.Dead)
                    return false;
            }
        ...
        /// <see cref="UnitTestProject.AtariRedundantMoveTest.AtariRedundantMoveTest_Scenario_XuanXuanQiJing_A36" />
        /// <see cref="UnitTestProject.AtariRedundantMoveTest.AtariRedundantMoveTest_Scenario_WuQingYuan_Q31670" />
        ...
        private static Boolean AtariRedundantAtNonKillableGroup(GameTryMove tryMove, Board b)
        {
            ...
            if (LinkHelper.IsAbsoluteLinkForGroups(currentBoard, tryBoard) && LinkHelper.GetPreviousMoveGroup(currentBoard, tryBoard).Any(n => n.Liberties.Count <= 2))
                return false;
            //check killer group
        ...
        public static Boolean SuicidalRedundantMove(GameTryMove tryMove)
        {
            ...
            else if (tryBoard.MoveGroupLiberties == 2)
            {
                if (SuicidalConnectAndDie(tryMove))
                    return true;
                if (SuicidalGroupNextToCorner(tryMove))
                    return true;
            }
        ...
        /// <summary>
        /// Suicidal group next to corner.
        /// <see cref="UnitTestProject.CheckForRecursionTest.CheckForRecursionTest_20260727_8" />
        /// <see cref="UnitTestProject.LeapMoveTest.LeapMoveTest_Scenario_WuQingYuan_Q31498" />
        /// Check diagonal cut <see cref="UnitTestProject.DailyGoProblems.DailyGoProblems_20221030_7" />
        /// Check diagonal link <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_Corner_A79" />
        /// </summary>
        private static Boolean SuicidalGroupNextToCorner(GameTryMove tryMove)
        {
            Board currentBoard = tryMove.CurrentGame.Board;
            Board tryBoard = tryMove.TryGame.Board;
            if (tryBoard.MoveGroup.Points.Count <= 2) return false;
            if (!tryBoard.IsPointNextToCorner()) return false;
            //check liberty at corner
            Point p = tryBoard.MoveGroup.Liberties.FirstOrDefault(n => tryBoard.CornerPoint(n));
            if (p.IsEmpty()) return false;
            //check connected liberties
            if (!tryBoard.GetStoneNeighbours(p).Any(q => tryBoard.MoveGroup.Liberties.Contains(q))) return false;
            //check atari target
            if (tryBoard.AtariTargets.Count != 1) return false;
            if (!KoHelper.IsKoFight(tryBoard, tryBoard.AtariTargets.First())) return false;
            //check connect and die
            if (!ImmovableHelper.CheckConnectAndDie(tryBoard)) return false;
            //check diagonal cut
            if (LinkHelper.FindDiagonalCut(tryBoard).Any()) return false;
            //check diagonal link
            if (LinkHelper.GetDiagonalGroupsWithoutCut(tryBoard).Any(n => !ImmovableHelper.CheckConnectAndDie(currentBoard, n)))
                return true;
            return false;
        }
        ...
        private static Boolean KoFightAtBloatedEye(Board tryBoard, Board currentBoard)
        {
            ...
            foreach (Point d in ImmovableHelper.GetDiagonalsOfTigerMouth(tryBoard, move, c))
            {
                ...
                if ((tryBoard.GetStoneNeighbours(d).Any(n => KoHelper.MakeKoFight(currentBoard, n, c).Item1) || KoHelper.IsKoFight(currentBoard, d, c).Item1))
                    return true;
            }
            return false;
        }
        ...
        public static Boolean OpponentSuicidalConnectAndDie(GameTryMove tryMove, GameTryMove opponentMove)
        {
            ...
            if (tryBoard.MoveGroup.Points.Count > 1 && LinkHelper.GetDiagonalGroupsWithoutCut(tryBoard).Any() && tryBoard.GetMoveLiberties().Any())
                return false;
            ...
            //check connect and die
            if (CheckConnectAndDieInOpponentSuicidal(tryMove, opponentMove))
                return false;
        ...
        private static Boolean CheckTwoPointGroupInOpponentSuicidal(GameTryMove tryMove, GameTryMove opponentMove, Board captureBoard)
        {
            ...
            if (GroupHelper.GetDirectKillerGroup(currentBoard, move, c.Opposite()) != null) return false;
        ...
        /// <summary>
        /// Check connect and die in opponent suicidal.
        /// <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_TianLongTu_Q2174_2" />
        ...
        /// </summary>
        private static Boolean CheckConnectAndDieInOpponentSuicidal(GameTryMove tryMove, GameTryMove opponentMove)
        {
            Board currentBoard = tryMove.CurrentGame.Board;
            Board tryBoard = tryMove.TryGame.Board;
            Board opponentBoard = opponentMove.TryGame.Board;
            ...
            foreach (Group group in tryBoard.OpponentGroupsAtStoneAndDiagonalNeighbour())
            {
                ...
                if (tryBoard.MoveGroup.Points.Count == 1 && tryBoard.CornerPoint())
                    return true;
            }
            return false;
        }
        ...
        /// Check two point move group <see cref="UnitTestProject.DailyGoProblems.DailyGoProblems_20260522_4" />
        ...
        private static Boolean CheckNonKillableInConnectAndDie(GameTryMove tryMove, Board captureBoard)
        {
            ...
            Point move = tryMove.Move;
            Content c = tryMove.MoveContent;
            //check two point move group
            if (tryBoard.MoveGroup.Points.Count >= 2 && WallHelper.TargetWithAnyNonKillableGroup(captureBoard, tryBoard.MoveGroup))
            {
                Boolean rc = tryBoard.GetGroupsFromStoneNeighbours().Any(n => n.Liberties.Count <= 2) && LinkHelper.FindDiagonalCut(tryBoard).Any();
                if (!rc) return true;
            }
        ...
        /// Check atari targets <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_GuanZiPu_A35" />
        ...
        /// Check diagonal group capture <see cref="UnitTestProject.DailyGoProblems.DailyGoProblems_20221028_7" />
        /// <see cref="UnitTestProject.DailyGoProblems.DailyGoProblems_20260625_7" />
        ...
        private static Boolean CheckImmovablePointAtDiagonal(GameTryMove tryMove, Board captureBoard)
        {
            ...
            if (!diagonals.Any())
            {
                ...
                {
                    if (!captureBoard.PointWithinMiddleArea(d)) continue;
                ...
                foreach (Point d in tryBoard.GetDiagonalNeighbours())
                {
                    ...
                    if (!ImmovableHelper.IsImmovablePoint(captureBoard, d, c.Opposite())) continue;
                    //check killer group
                    if (ngroups.Any(n => n.Points.Count > 1) && GroupHelper.IsSingleGroupWithinKillerGroup(captureBoard, tryBoard.MoveGroup))
                        return true;
                    ...
                    if (ngroups.Count == 1 && ngroups.First().Points.Count > 1)
                    {
                        ...
                        if (LinkHelper.GetDiagonalGroupsWithoutCut(tryBoard, ngroups.First()).Any(s => !WallHelper.IsHostileGroup(tryBoard, s)))
                            continue;
                        //check side point
            ...
            else
            {
                ...
                {
                    if (!captureBoard.PointWithinMiddleArea(p)) continue;
        ...
        private static Boolean CheckEmptyPointsAtStoneAndDiagonal(GameTryMove tryMove)
        {
            ...
            if (ngroups.Count == 1)
            {
                ...
                {
                    Board b = tryMove.CaptureBoard.MakeMoveOnNewBoard(e.Value, c);
                    if (b != null && KoHelper.IsKoFight(b, tryBoard.MoveGroup))
                        return false;
                }
                return true;
            }
        ...
        /// Check empty point at diagonal <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_WindAndTime_Q30241_2" />    
        ...
        /// Check non killable group at diagonal move <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_TianLongTu_Q16735" />
        /// Check connect and die at diagonal move <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_XuanXuanQiJing_A61" />
        ...
        private static Boolean CheckOnePointMoveDiagonalsInConnectAndDie(GameTryMove tryMove, Board captureBoard)
        {
            ...
            {
                //check non killable group at diagonal move
                if (!b.PointWithinMiddleArea() && WallHelper.IsNonKillableGroup(b))
                    return true;
                //check connect and die at diagonal move
        ...
        private static Boolean CheckOnePointMoveWithoutDiagonalsInConnectAndDie(GameTryMove tryMove, Board captureBoard)
        {
            ...
            foreach (Point p in tryBoard.GetDiagonalNeighbours().Where(n => tryBoard[n] == Content.Empty))
            {
                ...
                if (b.MoveGroup.Liberties.Count == 2)
                {
                    ...
                    if (KoHelper.IsKoFight(b2, q, c).Item1 || KoHelper.MakeKoFightFromEyePoint(b2, q, c).Item1)
                        return false;
                }
        ...
        /// <summary>
        /// Check capture move at empty space.
        /// Check opponent group <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_WuQingYuan_Q31453_2" />
        /// Check diagonal groups <see cref="UnitTestProject.DailyGoProblems.DailyGoProblems_20260726_8" />
        /// </summary>
        private static Boolean CheckCaptureMoveAtEmptySpace(GameTryMove tryMove, Board captureBoard)
        {
            Board tryBoard = tryMove.TryGame.Board;
            List<Group> ngroups = tryBoard.GetGroupsFromStoneNeighbours();
            if (captureBoard.MoveGroup.Points.Count != 1 || ngroups.Count != 1) return false;
            Point? v = LinkHelper.CheckPointsBetweenDiagonalsAtMove(captureBoard, Content.Empty);
            if (v == null || captureBoard[v.Value] != Content.Empty) return false;
            Group ngroup = ngroups.First();
        ...
        /// <see cref="UnitTestProject.DailyGoProblems.DailyGoProblems_20260822_8" />
        /// </summary>
        ...
        public static Boolean FindBloatedEyeSuicide(GameTryMove tryMove, Board captureBoard)
        {
            ...
            if (eyeGroups.Count == 1 && tryMove.EssentialSuicidalKillerFormation)
                return false;
        ...
        private static Boolean RedundantMultiPointMoveInConnectAndDie(GameTryMove tryMove, Board captureBoard)
        {
            ...
            if (tryBoard.GetStoneNeighbours(p).Any(q => tryBoard.MoveGroup.Liberties.Contains(q)))
            {
                ...
                //check for killer formation
                if (tryBoard.MoveGroup.Points.Count >= 3 && tryMove.EssentialSuicidalKillerFormation)
                    return false;
                return true;
            }
            return false;
        ...
        private static Boolean CheckDiagonalAndLibertyAtMove(GameTryMove tryMove, Board captureBoard)
        {
            ...
            //check connect and die
            if (CheckConnectAndDieForMultiPointMove(tryMove, captureBoard))
                return true;
        ...
        /// <summary>
        /// Check connect and die for multi-point move.
        /// <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_TianLongTu_Q16827_2" />
        /// Check killer formation <see cref="UnitTestProject.ImmovableTest.ImmovableTest_Scenario_XuanXuanGo_A26" />
        /// </summary>
        private static Boolean CheckConnectAndDieForMultiPointMove(GameTryMove tryMove, Board captureBoard)
        {
            Point move = tryMove.Move;
            Board currentBoard = tryMove.CurrentGame.Board;
            Board tryBoard = tryMove.TryGame.Board;
            Content c = tryMove.MoveContent;
            //check connect and die for previous groups
            if (LinkHelper.GetPreviousMoveGroup(currentBoard, tryBoard).Any(n => ImmovableHelper.CheckConnectAndDie(currentBoard, n))) return false;
            //check killer formation
        ...
        public static Boolean SuicideWithinRealEye(GameTryMove tryMove, Board capturedBoard)
        {
            ...
            {
                if (CheckThreeLibertyMoveForSuicideWithinRealEye(tryMove, capturedBoard))
                    return false;
            }
        ...
        /// <summary>
        /// Check three liberty move for suicide within real eye.
        /// <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_20221019_6" />
        /// </summary>
        private static Boolean CheckThreeLibertyMoveForSuicideWithinRealEye(GameTryMove tryMove, Board captureBoard)
        {
            Board currentBoard = tryMove.CurrentGame.Board;
            Board tryBoard = tryMove.TryGame.Board;
            Point move = tryMove.Move;
            Content c = tryMove.MoveContent;
            if (!EyeHelper.FindSemiSolidEye(captureBoard, move, c.Opposite())) return false;
        ...
        /// <summary>
        /// Check atari move in suicide real eye.
        ...
        /// Check one-point atari target <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_Phenomena_B18" />
        /// Check one neighbour group <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_XuanXuanGo_A27_3" />
        /// Check move at liberty <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_WuQingYuan_Q31640" />
        /// Check killer group at diagonal <see cref="UnitTestProject.SuicidalRedundantMoveTest.SuicidalRedundantMoveTest_Scenario_Corner_B33" />
        /// </summary>
        private static Boolean CheckAtariMoveInSuicideRealEye(GameTryMove tryMove, Board captureBoard)
        {
            ...
            if (captureBoard.GetGroupsFromStoneNeighbours(move, c).All(n => n.Liberties.Count > 2))
                return true;
            if (captureBoard.GetDiagonalNeighbours(move).Where(d => captureBoard[d] == Content.Empty).All(n => !captureBoard.OpponentGroupsAtStoneAndDiagonalNeighbour(n, c.Opposite()).Any(), true))
                return true;

            if (tryBoard.AtariTargets.Count == 1)
            {
                Group atariTarget = tryBoard.AtariTargets.First();
                //check one-point atari target
                if (atariTarget.Points.Count == 1 && LinkHelper.GetDiagonalGroups(tryBoard).Any())
                    return true;
                //check one neighbour group
                if (atariTarget.Points.Count > 1 && tryBoard.GetGroupsFromStoneNeighbours().Count == 1)
                    return true;
                //check move at liberty
                Board b = ImmovableHelper.MakeMoveAtLiberty(tryBoard, atariTarget);
                if (b != null && b.MoveGroupLiberties == 1)
                    return true;
            }
            //check killer group at diagonal
        ...
        public static Boolean CheckLeapMoveToTarget(GameTryMove tryMove, HashSet<Group> groups = null)
        {
            ...
            {
                List<Point> points = tryBoard.GetClosestPoints(p, c, 2).Where(r => tryBoard.GetGroupAt(r) != group).ToList();
        ...
        private static Boolean CheckOpponentForNeutralPointKillMoveOnly(GameTryMove tryMove)
        {
            ...
            Content c = tryBoard.MoveGroup.Content;
            //check opponent groups
            if (tryBoard.OpponentGroupsAtStoneAndDiagonalNeighbour().Count >= 2) return false;
            //check middle area
            if (!tryBoard.PointWithinMiddleArea())
            {
                //check one point move
                if (tryBoard.MoveGroup.Points.Count == 1) return false;
        ...
        /// Check real eye at diagonal <see cref="UnitTestProject.NeutralPointMoveTest.NeutralPointMoveTest_Scenario_XuanXuanGo_Q18500_4" />
        ...
        private static Boolean EssentialAtariAtCoveredPoint(GameTryMove tryMove)
        {
            ...
            if (KoHelper.IsKoFight(tryBoard, atariTarget))
            {
                ...
                if (diagonals.Count == 1)
                {
                    ...
                    if (EyeHelper.FindRealEyeWithinEmptySpace(tryBoard, s, c.Opposite()))
                        return false;
                }
                return true;
            }
        ...
        #region restore neutral moves
        /// <summary>
        /// Restore kill neutral move. Move restored on end game to kill survival group.
        ...
        /// </summary>
        public static void RestoreKillNeutralMove(Game g, List<GameTryMove> tryMoves, List<GameTryMove> redundantTryMoves)
        {
            ...
            neutralPointMoves.RemoveAll(n => !n.TryGame.Board.OpponentGroupsAtStoneAndDiagonalNeighbour().Any());
            if (neutralPointMoves.Count == 0) return;
        ...
        /// <summary>
        /// Restore survival neutral move. 
        /// <see cref="UnitTestProject.DailyGoProblems.DailyGoProblems_20260921_6" />
        /// </summary>
        public static void RestoreSurvivalNeutralMove(Game g, List<GameTryMove> tryMoves, List<GameTryMove> redundantTryMoves)
        {
            if (!g.GameInfo.MovablePointsOnly) return;
            List<GameTryMove> neutralPointMoves = redundantTryMoves.Where(e => e.IsNeutralPoint).ToList();
            GameTryMove tryMove = GetSpecificNeutralMove(g, neutralPointMoves);
            if (tryMove != null)
                tryMoves.Add(tryMove);
        }
        ...
        public static GameTryMove GetSpecificNeutralMove(Game g, List<GameTryMove> neutralPointMoves)
        {
            ...
            List<Group> immovableGroups = IsImmovableKill(g).ToList();
            if (immovableGroups.Any())
                ...
                tryMove = SpecificKillWithLibertyFight(g.Board, neutralPointMoves);
            return tryMove;
        }
        ...
        /// </summary>
        public static IEnumerable<Group> IsImmovableKill(Game g)
        {
            foreach (Group killerGroup in GroupHelper.GetKillerGroups(g.Board))
            {
        ...
        /// </summary>
        public static GameTryMove SpecificKillWithLibertyFight(Board board, List<GameTryMove> neutralPointMoves)
        {
            foreach (GameTryMove tryMove in neutralPointMoves)
            {
                ...
                foreach (Group targetGroup in tryBoard.GetGroupsFromStoneNeighbours())
                {
                    ...
                    List<Group> kgroups = GroupHelper.GetKillerGroups(board).Where(gr => board.GetNeighbourGroups(gr).Contains(board.GetCurrentGroup(targetGroup))).ToList();
            ...
            foreach (Point d in diagonalPoints)
            {
                ...
                if (opponentMove == null)
                {
                    ...
                    if (diagonalKillerGroup != null)
                    {
                        ...
                        List<Group> opponentGroups = currentBoard.OpponentGroupsAtStoneAndDiagonalNeighbour(d, c.Opposite()).Where(n => GroupHelper.GetDirectKillerGroup(currentBoard, n.Points.First(), c.Opposite()) == diagonalKillerGroup).ToList();
        ...
        private static Boolean CheckCoveredEyeAtTigerMouth(GameTryMove tryMove, Board capturedBoard, GameTryMove opponentMove = null)
        {
            ...
            if (KillerFormationHelper.TigerMouthAtDiagonal(tryBoard).Any(n => n.Points.Count <= 1 || !GroupHelper.IsSingleGroupWithinKillerGroup(capturedBoard, n)))
                return false;

            if (capturedBoard.MoveGroup.Points.Count == 1)
            {
                //check move next to corner
                if (capturedBoard.CornerPoint() && CheckSuicideKoMoveAtCorner(tryBoard))
                    return false;
        ...
        /// <summary>
        /// Check suicide ko move at corner.
        /// Check connect and die <see cref="UnitTestProject.SurvivalTigerMouthMoveTest.RedundantTigerMouthMove_ScenarioHighLevel23" /> 
        /// </summary>
        private static Boolean CheckSuicideKoMoveAtCorner(Board tryBoard)
        {
            Point move = tryBoard.Move.Value;
            Content c = tryBoard.MoveGroup.Content;
            Point corner = tryBoard.GetStoneNeighbours().FirstOrDefault(n => tryBoard.CornerPoint(n));
            if (corner.IsEmpty() || tryBoard[corner] != Content.Empty) return false;
            Point q = tryBoard.GetStoneNeighbours(corner).First(n => !n.Equals(move));
            if (tryBoard[q] != Content.Empty) return false;
            //make ko move
        ...
        private static Boolean CheckWeakGroupAtTigerMouth(GameTryMove tryMove, Board capturedBoard)
        {
            ...
            if (diagonals.Any(n => ImmovableHelper.IsSnapback(tryBoard, tryBoard.MoveGroup, n)))
                return true;
            //check diagonal at move
            if (capturedBoard.GetMoveLiberties().Count == 1 && KillerFormationHelper.TigerMouthAtDiagonal(tryBoard).Any())
                return true;
            //check hostile group
        ...
        public static Boolean RedundantKoMove(GameTryMove tryMove)
        {
            ...
            if (KoHelper.IsKoFight(tryBoard))
            {
                ...
                {
                    //check pre-ko moves
                    if (CheckPreKoMove(tryMove)) return false;
                    //check double ko
        ...
        public static Boolean CheckPreKoMove(GameTryMove tryMove)
        {
            ...
            //check atari target
            if (tryBoard.AtariTargets.Count != 1) return true;
            Group group = tryBoard.AtariTargets.First();
            //check diagonal group with cut
            ...
            if (ImmovableHelper.CheckConnectAndDie(b, group, false))
                return true;
            return false;
        }
        ...
        {
            //single point move
            if (RedundantNonSuicidalSinglePointMove(tryMove))
                return true;

            //single point move for opponent
            if (tryMove.OpponentMove != null && RedundantNonSuicidalSinglePointMove(tryMove.OpponentMove, tryMove))
                return true;

            //multi-point move
            if (RedundantNonSuicidalMultiPointMove(tryMove))
                return true;

            //multi-point move for opponent
            if (tryMove.OpponentMove != null && RedundantNonSuicidalMultiPointMove(tryMove.OpponentMove, tryMove))
                return true;
            return false;
        }
        ...
        public static Boolean RedundantNonSuicidalSinglePointMove(GameTryMove tryMove, GameTryMove opponentMove = null)
        {
            ...
            //check neighbour groups
            if (tryBoard.OpponentGroupsAtStoneAndDiagonalNeighbour().Count != 1) return false;
        ...
        public static Boolean RedundantNonSuicidalMultiPointMove(GameTryMove tryMove, GameTryMove opponentMove = null)
        {
            ...
            if (CheckNonKillableForMultiPointMove(tryMove, opponentMove))
                return true;

            //check multi-point move
            if (opponentMove != null) return false;
            if (tryMove.IncreasedKillerGroups || tryMove.EssentialSuicidalKillerFormation) return false;
            //no opponent groups
            if (tryBoard.OpponentAtStoneNeighbour().Any()) return false;
        ...
        /// Check non killable for multi-point move.
        /// <see cref="UnitTestProject.DailyGoProblems.DailyGoProblems_20260902_8" />
        /// Check non killable groups at stone neighbour <see cref="UnitTestProject.DailyGoProblems.DailyGoProblems_20260921_7" />
        /// Check opponent move group <see cref="UnitTestProject.NeutralPointMoveTest.NeutralPointMoveTest_Scenario_XuanXuanGo_A26_2" />
        /// Check opponent groups <see cref="UnitTestProject.RedundantNonSuicidalMoveTest.RedundantNonSuicidalMoveTest_Scenario_Corner_A80" />
        /// Check diagonal point <see cref="UnitTestProject.RedundantNonSuicidalMoveTest.RedundantNonSuicidalMoveTest_Scenario_WindAndTime_Q30332" />
        /// </summary>
        private static Boolean CheckNonKillableForMultiPointMove(GameTryMove tryMove, GameTryMove opponentMove = null)
        {
            Board currentBoard = tryMove.CurrentGame.Board;
            Board tryBoard = tryMove.TryGame.Board;
            Point move = tryMove.Move;
            Content c = tryBoard.MoveGroup.Content;
            //check non killable groups at stone neighbour
            List<Point> stoneNeighbours = tryBoard.GetStoneNeighbours().Where(n => tryBoard[n] == c && WallHelper.IsNonKillableFromSetupMoves(currentBoard, currentBoard.GetGroupAt(n)) && !tryBoard.OpponentAtStoneNeighbour(n).Any()).ToList();
            if (stoneNeighbours.Count < 2) return false;
            HashSet<Group> opponentGroups = tryBoard.OpponentGroupsAtStoneAndDiagonalNeighbour();
            if (opponentGroups.Count <= 1)
            {
                //check opponent move group
                if (opponentMove != null)
                {
                    Board opponentBoard = opponentMove.TryGame.Board;
                    if (opponentBoard.MoveGroup.Points.Count == 2 && opponentBoard.GetStoneNeighbours().Any(n => opponentBoard[n] == Content.Empty && ImmovableHelper.ConnectAndDieMove(opponentBoard, n, c).Item1))
                        return false;
                }
                return true;
            }
            //check opponent groups
            if (opponentGroups.Any(n => n.Liberties.Count <= 2)) return false;

            //check diagonal point
            Point p = tryBoard.GetDiagonalNeighbours().Where(n => !tryBoard.GetStoneNeighbours(n).Intersect(stoneNeighbours).Any()).FirstOrDefault();
            if (p.IsEmpty()) return false;
            if (tryBoard.OpponentGroupsAtStoneAndDiagonalNeighbour(p, c.Opposite()).Where(n => !n.Equals(tryBoard.MoveGroup)).Any())
                return false;
            return true;
        }
