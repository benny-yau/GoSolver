...
namespace Go
{
    public class RedundantMoveHelper
    {
        ...
        public static Boolean SuicidalConnectAndDie(GameTryMove tryMove)
        {
            ...
            {
                Group weakGroup = groups.FirstOrDefault(group => (group.Points.Count >= 2 && group.Liberties.Count <= 2 && !WallHelper.IsNonKillableGroup(tryBoard, group)));
                if (weakGroup != null && groups.Any(group => group.Liberties.Count > 2))
                {
                    if (tryBoard.GetNeighbourGroups(weakGroup).All(g => !WallHelper.IsNonKillableGroup(tryBoard, g)))
                        return false;
                }
            }
        ...
        public static Boolean SuicidalWithinNonKillableGroup(GameTryMove tryMove)
        {
            ...
            if (tryBoard.GetDiagonalNeighbours(move.x, move.y).Any(n => tryBoard[n] == c) && groups.Any(group => group.Points.Count == 2 && group.Liberties.Count <= 2))
                return false;
            return true;
        }
        ...
        private static Boolean CheckOppositeContentForFillerMove(GameTryMove tryMove, Boolean isKillerGroup = false)
        {
            ...
            Point move = tryBoard.Move.Value;
            Content c = tryMove.MoveContent;
            if (tryBoard.CornerPoint(tryMove.Move)) return false;
            if (!isKillerGroup)
            {
